...
const BANK_KEYS = [
  { label: "BDO", code: "0522", logoUrl: "/bdo.jpg" },
  { label: "MBTC", code: "0102", logoUrl: "/mbtc8.png" },
];

const bankColors = ["#0b2972"];

const role = "assistant";

const OCSIAssistantCashPositionReportPage = () => {
  ...
  const calculateBankTotals = (transactions) => {
    ...
    transactions.forEach((t) => {
      const match = BANK_KEYS.find((b) => t.bank.includes(b.code));
      if (match) {
        totals[match.label] = (totals[match.label] || 0) + parseFloat(t.amount);
      } else {
        console.warn("🚫 Transaction ignored for unlisted bank:", t.bank);
      }
    });

    BANK_KEYS.forEach((b) => {
      if (!totals[b.label]) totals[b.label] = 0;
    });
  ...
  const handleSaveEndingBalance = async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/OCSI/transactions/ending-balance",
        {
          method: "POST",
          headers: {
  ...
  const handleExportToExcel = async () => {
    ...
    // ==== Title Row ====
    worksheet.addRow([`Assistant OCSI Cash Position Report - ${reportDate}`]);
    ...
    a.href = url;
    a.download = `Assistant_OCSI_Cash_Report_${reportDate}.xlsx`;
    a.click();
  ...
  return (
    ...
    >
      <h1 className="text-xl font-bold mb-2 mt-4">
        Optichain Solutions Inc. - ASSISTANT
      </h1>
      ...
      {reportDate && (
        <>
          <p className="mb-6 mt-4 font-medium">
          ...
          <table className="w-full    border border-gray-700 mb-10 text-sm">
            <thead className="bg-gray-100 ">
              <tr className="w-full">
                <th className="border-gray-700 px-2 py-2 text-left w-40 font-medium text-white uppercase tracking-wider"></th>
              ...
              <tr>
                <th className="border border-gray-700 px-2 py-2 font-normal text-left">
                  Account No. Ending
                </th>
                {BANK_KEYS.map((b) => (
                  <th
                ...
                <th className="border px-2 py-2 text-center border-gray-700">
                  —
                </th>
              </tr>
            </thead>
            <tbody>
              {/* Beginning Balance */}
              <tr>
                <td className="border px-2 py-2 font-normal border-gray-700 ">
                  Beginning Balance
                </td>
                {BANK_KEYS.map((b) => (
                  <td
                ...
                <td className="border px-2 py-2 text-gray-800 text-center font-semibold border-gray-700">
              ...
              {sortByBankAndTime(deposits).map((t, idx) => {
                const matchesBank = BANK_KEYS.some((b) =>
                  t.bank.includes(b.code),
                );

                // If no bank matches, skip rendering row
                if (!matchesBank) return null;

                return (
                  <tr key={`deposit-${idx}`}>
                    {/* Description cell */}
                    <td className="border px-2 py-2 text-left w-[40em] border-gray-700">
                      {t.description || ""}
                    </td>
                    ...
                    {BANK_KEYS.map((b) => (
                      <td
                        key={b.label}
                        className="px-6 py-4 whitespace-nowrap text-sm text-green-600 text-center border border-gray-700"
                      >
                        {t.bank.includes(b.code)
                    ...
                    <td className="border px-2 py-2 text-green-600 text-right font-semibold border-gray-700">
              ...
              <tr className="bg-green-100 font-semibold border border-gray-700">
                <td className="border px-2 py-2 border-gray-700">
                  Sub-Total Deposits
                </td>
                {BANK_KEYS.map((b) => (
                  <td
                    key={b.label}
                    className="border px-6 py-4 whitespace-nowrap text-sm text-gray-800 font-semibold text-center border-gray-700"
                  >
                    {formatCurrency(depositTotals[b.label])}
                  </td>
                ))}
                <td className="border px-6 py-4 whitespace-nowrap text-sm text-gray-800 font-semibold text-center border-gray-700">
                  {formatCurrency(sumObjectValues(depositTotals))}
                </td>
              </tr>
              <tr className="font-semibold border border-gray-700">
                <td className="px-2 py-2 border-gray-700">{"‎ "}</td>
                <td className="px-2 py-2 border-gray-700">{""}</td>
              </tr>
              {/* Total (Beginning + Deposit) */}
              <tr className="bg-green-100 font-bold border border-gray-700">
                <td className="border px-2 py-2 border-gray-700">TOTAL</td>
                {BANK_KEYS.map((b) => (
                  <td
                    key={b.label}
                    className="border px-2 py-2 text-green-900 text-center border-gray-700"
                  >
                    {formatCurrency(
                ...
                <td className="border px-2 py-2 text-green-900 text-center font-bold border-gray-700">
                  {formatCurrency(
                    sumObjectValues(depositTotals) +
                      sumObjectValues(beginningBalance),
                  )}
              ...
              <tr>
                <th className="border px-2 py-2 text-[12px] font-normal text-left border-gray-700">
                  LESS: DISBURSEMENT
                </th>
                {BANK_KEYS.map((b, index) => (
                  <th
                ...
                <th className="border px-2 py-2 text-center bg-gray-700 text-white font-semibold">
                  TOTAL
                </th>
              </tr>
              ...
              {sortByBankAndTime(expenses).map((t, idx) => {
                ...
                return (
                  <tr key={`expense-${idx}`}>
                    {/* Description cell */}
                    <td className="border px-2 py-2 text-left border-gray-700">
                      {t.description || ""}
                    </td>
                    ...
                    {BANK_KEYS.map((b) => (
                      <td
                        key={b.label}
                        className="border px-6 py-4 whitespace-nowrap text-sm text-red-600 text-center border-gray-700"
                      >
                        {t.bank.includes(b.code)
                          ? formatCurrency(t.amount)
                          : ""}
                    ...
                    {/* Final extra cell (to align with deposits table) */}
                    <td className="border px-6 py-4 whitespace-nowrap text-sm text-red-600 text-center border-gray-700">
                      {/* {formatCurrency(parseFloat(t.amount))} */}
                    </td>
                  </tr>
                );
              ...
              <tr className="bg-red-50 font-semibold">
                ...
                {BANK_KEYS.map((b) => (
                  <td
                    key={b.label}
                    className="border px-6 py-4 whitespace-nowrap text-sm text-red-600 font-semibold text-center border-gray-700"
                  >
                    {formatCurrency(expenseTotals[b.label])}
                  </td>
                ))}
                <td className="border px-2 py-2 text-red-800 text-center font-bold border-gray-700">
              ...
              <tr className="bg-yellow-50 font-semibold">
                ...
                {BANK_KEYS.map((b) => (
                  <td
                    key={b.label}
                    className="border px-6 py-4 whitespace-nowrap text-sm font-bold text-blue-800 text-center border-gray-700"
                  >
                    {formatCurrency(
                      depositTotals[b.label] +
                        beginningBalance[b.label] -
                ...
                <td className="border px-6 py-4 whitespace-nowrap text-sm font-bold text-blue-800 text-center border-gray-700">
                  {formatCurrency(
                    sumObjectValues(depositTotals) +
                      sumObjectValues(beginningBalance) -
                      sumObjectValues(expenseTotals),
...
export default OCSIAssistantCashPositionReportPage;
